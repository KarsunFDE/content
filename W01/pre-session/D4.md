...
> Read after Thu's first LLM PRs land and before Fri morning. ~50 min. Fri is dense (12 topics including explicit HITL + First PR Adversarial Review) — this pre-session prepares the foundation.

## 1. Structured JSON output — Pydantic and beyond (15 min)
...
## 3. Context engineering — system prompt architecture (10 min)

The system prompt is an *architectural artifact*, not a string literal. It has named sections:
...
Treat system prompts as code. They live in version control. They have tests. They are ADR-worthy.

## 4. Dynamic context assembly (5 min)

The W2 RAG layer adds dynamic context — retrieved chunks injected into the system prompt at request time. Friday's PR prepares for this by:
...
## 5. Context compression patterns (3 min)

When the grounding context is large (full FAR clauses), the model hits its context window. Patterns Fri previews (deep work W2):
...
## 6. Prompt evaluations + lifecycle management (5 min)
...
## 7. Explicit HITL framing — Friday's named topic (5 min)
...
## 8. Async FastAPI integration with Spring Boot (5 min)
...
## 9. First PR Adversarial Review (Light) — what to expect Fri (3 min)
